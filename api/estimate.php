...
if($_SERVER['REQUEST_METHOD']=="POST"){
    $result = mysqli_query($db, "INSERT INTO  `estimate` (`submissionid`, `estimate`) VALUES (".
        mysqli_real_escape_string($db, $_POST['sid']).",".
        mysqli_real_escape_string($db, $_POST['estimate']).")");
}

$result2 = mysqli_query($db, "SELECT estimate as estimateValue, count(1) as numberOfEstimates FROM `estimate` WHERE submissionid = ".
    mysqli_real_escape_string($db, $_REQUEST['sid'])." GROUP BY estimate;");
$estimates = mysqli_fetch_all($result2, MYSQLI_ASSOC);

$result3 = mysqli_query($db, "SELECT realop FROM `submission` WHERE sid = ".
